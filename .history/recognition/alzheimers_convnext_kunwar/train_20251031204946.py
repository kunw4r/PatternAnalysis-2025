...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        # Early stopping
        # Stop if: standard patience reached OR high accuracy (99%+) with 5 epochs no improvement
        if patience_counter >= patience or (best_val_acc >= 99.0 and patience_counter >= 5):
            if best_val_acc >= 99.0 and patience_counter >= 5:
                print(f"{'='*80}")
                print(f"🎯 EARLY STOPPING: High accuracy achieved!")
                print(f"Validation accuracy {best_val_acc:.2f}% >= 99% threshold")
